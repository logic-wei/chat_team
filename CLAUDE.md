...
**`image_detail` knob.** Defaults to `settings.llm.default_image_detail = "high"` (~1600 tokens per 1024² image on `gpt-4o`-class models). Override per role with `llm.image_detail: low|high|auto` in the role YAML. If you keep many images in context, bump `default_history_token_budget` (compactor only counts the `[图:<basename>]` placeholder, NOT the actual vision tokens — so a chat with several `high`-detail images can drift past the budget without triggering compaction). The provider stamps `detail` on every `image_url` part it builds; `image_base_dir` is plumbed from `session.cwd` so `./inbox/<file>` paths in history resolve correctly.
...
**Role** — drop a YAML in `src/chat_team/roles/builtin/` (committed) or `~/.chat_team/roles/` (user override, takes precedence). Required fields: `name`, `system_prompt`, `tools` (a subset of registered tool names). Optional: `display_name`, `welcome_message` (used for `enter_chat`), `llm.{model,temperature,history_token_budget,image_detail}`. No code changes needed — `RoleRegistry.load` picks it up and `transfer_to_employee`'s enum is rebuilt from `roles.names()`.
