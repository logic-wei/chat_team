...
sequenceDiagram
    ...
    rect rgb(214, 232, 240)
        Note over Dev,WeCom: 连接建立阶段
        Dev->>WeCom: 建立 WebSocket 连接 (aibot_subscribe)
        WeCom-->>Dev: 连接建立成功
    end

    rect rgb(238, 229, 210)
...
![企业微信截图_4df4a527-66cd-4422-85e2-62ab51f5e0b6.png](images/robot_config.png)
...
sequenceDiagram
    ...
    Dev->>WX: 1. 发起 WebSocket 连接\nwss://openws.work.weixin.qq.com
    WX-->>Dev: 2. WebSocket 握手成功

    Dev->>WX: 3. 发送订阅请求（aibot_subscribe）\n携带 BotID 和 Secret
    Note right of WX: 4. 校验凭证
    WX-->>Dev: 5. 返回订阅结果

    Note over Dev,WX: 连接建立完成，开始接收回调
```
...
| 消息类型 | msgtype | 说明 |
| --- | --- | --- |
| [文本消息](https://developer.work.weixin.qq.com/document/path/100719#%E6%96%87%E6%9C%AC%E6%B6%88%E6%81%AF) | text | 用户发送的文本内容 |
| [图片消息](https://developer.work.weixin.qq.com/document/path/100719#%E5%9B%BE%E7%89%87%E6%B6%88%E6%81%AF) | image | 用户发送的图片，仅支持单聊 |
| [图文混排](https://developer.work.weixin.qq.com/document/path/100719#%E5%9B%BE%E6%96%87%E6%B7%B7%E6%8E%92%E6%B6%88%E6%81%AF) | mixed | 用户发送的图文混排内容 |
| [语音消息](#支持的消息类型) | voice | 用户发送的语音（转为文本），仅支持单聊 |
| [文件消息](#支持的消息类型) | file | 用户发送的文件，仅支持单聊 |
| [视频消息](#支持的消息类型) | video | 用户发送的视频，仅支持单聊 |
...
| 事件类型 | eventtype | 说明 |
| --- | --- | --- |
| [进入会话事件](#进入会话事件) | enter_chat | 用户当天首次进入机器人单聊会话 |
| [模板卡片事件](#模板卡片事件) | template_card_event | 用户点击模板卡片按钮 |
| [用户反馈事件](#用户反馈事件) | feedback_event | 用户对机器人回复进行反馈 |
...
| body | object | 是 | 消息内容，详见[回复欢迎语](#回复欢迎语) |
...
> 注意：从流式消息发送开始，需在 10 分钟内完成所有刷新并设置 finish=true，否则消息将自动结束。
...
<!-- 序列图/流程图 -->
> 图示: 开发者服务 → 企业微信 → 用户 → @机器人发消息 → aibot_msg_callback (req_id=xxx) → 生成唯一的 stream.id → aibot_respond_msg (req_id=xxx) → (stream.id=abc, finish=false) → content="正在查询..." → 首次使用该 stream.id → 创建新的流式消息 → 展示流式消息 → content="广州今日天气：29度..." → 继续使用相同 stream.id → 刷新消息内容 → 更新流式消息 → content="广州今日天气：29度，多云，降雨概率60%..." → (stream.id=abc, finish=true) → content="广州今日天气：29度，多云，降雨概率60%，建议携带雨具。" → finish=true → 流式消息结束 → 展示最终消息 → 10 → 10分钟超时限制：从首次发送开始计时
...
| body | object | 是 | 消息内容，详见[回复用户消息](#回复普通消息)中的流式消息和模块卡片消息。不支持流式消息+模板卡片的组合消息 |
...
| headers.req_id | string | 是 | 透传事件回调中的 req_id |
| body | object | 是 | 消息内容，详见[更新模板卡片](#更新模板卡片) |
...
| 消息类型 | msgtype | 说明 |
| --- | --- | --- |
| 模板卡片 | template_card | 模板卡片消息，详见[模板卡片](#模板卡片消息) |
| Markdown | markdown | Markdown 格式消息，详见[markdown](#markdown消息) |
| 文件消息 | file | 普通文件消息，详见[文件消息](#文件消息) |
| 图片消息 | image | 图片消息，详见[图片消息](#图片消息) |
| 语音消息 | voice | 语音消息，详见[语音消息](#语音消息) |
| 视频消息 | video | 视频消息，详见[视频消息](#视频消息) |
...
| markdown.feedback.id | String | 否 | 若字段不为空值，回复的消息被用户反馈时候会触发[回调事件](#用户反馈事件)。有效长度为 256 字节以内，必须是 utf-8 编码。 |
...
| template_card | Object | 是 | 模板卡片结构体，参考[模板卡片类型](#模板卡片消息)中类型说明 |
| template_card.feedback.id | String | 否 | 若字段不为空值，回复的消息被用户反馈时候会触发[回调事件](#用户反馈事件)。有效长度为 256 字节以内，必须是 utf-8 编码。 |
...
| media_id | 是 | 文件id，可以调用[上传临时素材](#上传临时素材)接口获取 |
...
| media_id | 是 | 图片媒体文件id，可以调用[上传临时素材](#上传临时素材)接口获取 |
...
| media_id | 是 | 语音文件id，可以调用[上传临时素材](#上传临时素材)接口获取 |
...
| media_id | 是 | 视频媒体文件id，可以调用[上传临时素材](#上传临时素材)接口获取 |
...
sequenceDiagram
    ...
    rect rgb(235, 242, 250)
        Note over C,S: STEP 1：初始化上传
        C->>S: aibot_upload_media_init\n{type, filename, total_size, total_chunks, md5}
        S-->>C: {upload_id}
    end

    rect rgb(237, 247, 237)
        Note over C,S: STEP 2：分片上传
        loop 每个分片 i = 0, 1, ..., total_chunks - 1
            C->>S: aibot_upload_media_chunk\n{upload_id, chunk_index, base64_data}
            S-->>C: 成功
        end
    end

    rect rgb(247, 242, 228)
        Note over C,S: STEP 3：完成上传
        C->>S: aibot_upload_media_finish\n{upload_id}
        S-->>C: {type, media_id, created_at}
    end

    Note over C: 使用 media_id 发送\n文件/图片/语音/视频消息
```
